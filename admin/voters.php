            ...
            <div class="box-body">
              <table id="example1" class="table ">
                <thead>
                  ...
                  <th>Firstname</th>
                  <th>Photo</th>
                  <th>Voter ID</th> <!-- Changed header from 'Voters ID' to 'ID' -->
                  <th>Tools</th>
                </thead>
                <tbody>
                  <?php
                    $sql = "SELECT * FROM voters";  // No need to fetch 'voters_id' anymore
...
function getRow(id){
  $.ajax({
    ...
    success: function(response){
      ...
      $('#edit_username').val(response.username); // Added username field in modal edit
    }
  });
}
</script>
